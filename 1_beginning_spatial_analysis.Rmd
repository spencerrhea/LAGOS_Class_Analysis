...
minnesota_lakes <- mutate(minnesota_lakes, state = "minnesota")

iowa_lakes <- mutate(iowa_lakes, state = "iowa") 

both <- rbind(minnesota_lakes, iowa_lakes) 

ggplot(both, aes(lake_area_ha)) +
  geom_histogram() +
  scale_x_log10() +
  facet_wrap(~state)
```
...
```{r}
iowa_illinois_lakes<- iowa_illinois_lakes %>%
  arrange(lake_area_ha)
mapview(iowa_illinois_lakes, zcol= 'lake_area_ha')
```
...
#FLow rates or discharge from stream gauges 
